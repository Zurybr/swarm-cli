...
import { SkillTestHarness, SkillTestAssertionError } from '../../../src/testing/skill-test-harness';
...
// Simple test skill for testing the harness
...
class TestMultiCallSkill {
  ...
  async execute(input: { queries: string[] }): Promise<{ results: string[] }> {
...
describe('SkillTestHarness', () => {
  describe('basic execution', () => {
    it('executes skill with mocked LLM response', async () => {
      const harness = new SkillTestHarness(
        (llm) => new TestCodeAnalysisSkill(llm)
      );
  ...
  describe('LLM call assertions', () => {
    ...
    it('expectLLMCallContaining throws when substring not found', async () => {
      ...
      }).toThrow(SkillTestAssertionError);
    });

    it('expectLLMCallCount verifies number of calls', async () => {
      const harness = new SkillTestHarness(
        (llm) => new TestMultiCallSkill(llm)
      );
  ...
  describe('withFixtures', () => {
    it('loads multiple fixtures at once', async () => {
      ...
      const harness1 = new SkillTestHarness((llm) => ({
        execute: async () => {

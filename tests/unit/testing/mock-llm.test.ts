...
describe('MockLLMClient', () => {
  ...
  describe('fixture matching', () => {
    ...
    test('throws error when no fixture matches (fallbackMode: error)', async () => {
      ...
      // Act & Assert
      await expect(
        client.complete({ prompt: 'unmatched prompt' })
      ).rejects.toThrow('No fixture found for prompt');
    });
  });
...
describe('FixtureLoader', () => {
  ...
  const testFixtures: LLMResponseFixture[] = [
    {
      id: 'fixture-1',
  ...
  beforeEach(() => {
    loader = new FixtureLoader(testFixtures);
  });
  ...
  describe('fixture management', () => {
    test('add inserts new fixture', () => {
      // Arrange
      const newFixture: LLMResponseFixture = {
        id: 'new-fixture',
  ...
  describe('empty loader', () => {
    test('works with no initial fixtures', () => {
      // Arrange
      const emptyLoader = new FixtureLoader();
...
describe('SecurityReviewFixtures', () => {
  ...
  beforeEach(() => {
    loader = new FixtureLoader(SecurityReviewFixtures);
  });

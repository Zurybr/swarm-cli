...
const storage_1 = require("@/hive/storage");
...
describe('Storage', () => {
    ...
    describe('init', () => {
        it('should create storage directories', async () => {
            ...
            const newStorage = new storage_1.Storage(config);
    ...
    describe('save', () => {
        ...
        it('should write atomically using temp file', async () => {
            const cell = cell_1.Cell.create({ title: 'Atomic Test' });
            await storage.save(cell);
    ...
    describe('load', () => {
        it('should load a cell by ID', async () => {
            const cell = cell_1.Cell.create({ title: 'Test Cell', priority: 5 });
            await storage.save(cell);
            const result = await storage.load(cell.id);
            expect(result.success).toBe(true);
            expect(result.data).toBeInstanceOf(cell_1.Cell);
            expect(result.data.id).toBe(cell.id);
    ...
    describe('delete', () => {
        it('should delete a cell', async () => {
            const cell = cell_1.Cell.create({ title: 'To Delete' });
            await storage.save(cell);
    ...
    describe('exists', () => {
        it('should return true for existing cell', async () => {
            const cell = cell_1.Cell.create({ title: 'Exists' });
            await storage.save(cell);
            const exists = await storage.exists(cell.id);
    ...
    describe('listAll', () => {
        ...
        it('should list all cell IDs', async () => {
            ...
            const cell3 = cell_1.Cell.create({ title: 'Cell 3' });
    ...
    describe('loadAll', () => {
        it('should load all cells', async () => {
            const cell1 = cell_1.Cell.create({ title: 'Cell 1', type: 'epic' });
            const cell2 = cell_1.Cell.create({ title: 'Cell 2', type: 'task' });
            await storage.save(cell1);
            await storage.save(cell2);
    ...
    describe('query', () => {
        beforeEach(async () => {
            ...
            const epic = cell_1.Cell.create({ title: 'Epic 1', type: 'epic', status: 'open', priority: 10 });
            const task1 = cell_1.Cell.create({
                title: 'Task 1',
                type: 'task',
                status: 'in_progress',
                priority: 5,
            ...
            const task2 = cell_1.Cell.create({
                title: 'Task 2',
                type: 'task',
                status: 'open',
                priority: 3,
    ...
    describe('getNextReady', () => {
        ...
        it('should return highest priority open cell', async () => {
            const cell1 = cell_1.Cell.create({ title: 'Low', status: 'open', priority: 1 });
            const cell2 = cell_1.Cell.create({ title: 'High', status: 'open', priority: 10 });
            const cell3 = cell_1.Cell.create({ title: 'Medium', status: 'open', priority: 5 });
            await storage.save(cell1);
            await storage.save(cell2);
            await storage.save(cell3);
        ...
        it('should not return non-open cells', async () => {
            const open = cell_1.Cell.create({ title: 'Open', status: 'open' });
            const inProgress = cell_1.Cell.create({ title: 'In Progress', status: 'in_progress' });
    ...
    describe('getStats', () => {
        ...
        it('should calculate statistics', async () => {
            await storage.save(cell_1.Cell.create({ title: '1', type: 'epic', status: 'open' }));
            await storage.save(cell_1.Cell.create({ title: '2', type: 'task', status: 'open' }));
            await storage.save(cell_1.Cell.create({ title: '3', type: 'task', status: 'in_progress' }));
            await storage.save(cell_1.Cell.create({ title: '4', type: 'bug', status: 'completed' }));
            const stats = await storage.getStats();
    ...
    describe('clear', () => {
        it('should remove all cells', async () => {
            await storage.save(cell_1.Cell.create({ title: '1' }));
            await storage.save(cell_1.Cell.create({ title: '2' }));
            await storage.clear();
            const ids = await storage.listAll();

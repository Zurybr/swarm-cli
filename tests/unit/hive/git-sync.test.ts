...
import { GitSync, GitSyncError } from '@/hive/git-sync';
import { Cell } from '@/hive/cell';
import { StorageConfig, DEFAULT_STORAGE_CONFIG } from '@/hive/types';
...
describe('GitSync', () => {
  ...
  beforeEach(async () => {
    ...
    gitSync = new GitSync(config, testDir);
    await gitSync.init();
  });
  ...
  describe('init', () => {
    ...
    it('should detect non-git directory', async () => {
      ...
      const sync = new GitSync(config, nonGitDir);
      await sync.init();
    ...
    it('should be disabled when enableGit is false', async () => {
      ...
      const sync = new GitSync(disabledConfig, testDir);
      await sync.init();
  ...
  describe('commit', () => {
    ...
    it('should use custom message when provided', async () => {
      const cell = Cell.create({ title: 'Test Cell', id: 'test-123' });
      fs.writeFileSync(path.join(config.baseDir, 'test.txt'), 'test');
  ...
  describe('rollback', () => {
    it('should rollback to previous commit', async () => {
      ...
      const firstCommit = execSync('git rev-parse HEAD', { cwd: testDir, encoding: 'utf-8' }).trim();

...
describe('Cell', () => {
    describe('create', () => {
        ...
        it('should create a cell with custom values', () => {
            ...
            const cell = cell_1.Cell.create(options);
        ...
        it('should generate unique IDs', () => {
            const cell1 = cell_1.Cell.create({ title: 'Cell 1' });
            const cell2 = cell_1.Cell.create({ title: 'Cell 2' });
            expect(cell1.id).not.toBe(cell2.id);
        });
        it('should include type prefix in ID', () => {
            const epic = cell_1.Cell.create({ title: 'Epic', type: 'epic' });
            const bug = cell_1.Cell.create({ title: 'Bug', type: 'bug' });
    ...
    describe('fromJSON', () => {
        it('should rehydrate cell from JSON string', () => {
            const original = cell_1.Cell.create({
                title: 'Original',
            ...
            const restored = cell_1.Cell.fromJSON(json);
        ...
        it('should rehydrate cell from data object', () => {
            ...
            const cell = cell_1.Cell.fromJSON(data);
            expect(cell.id).toBe('test-123');
            expect(cell.title).toBe('Test');
        });
        it('should throw on missing required fields', () => {
            expect(() => cell_1.Cell.fromJSON({})).toThrow(cell_1.CellError);
            expect(() => cell_1.Cell.fromJSON({ id: 'test' })).toThrow(cell_1.CellError);
            expect(() => cell_1.Cell.fromJSON({ id: 'test', title: 'Test' })).toThrow(cell_1.CellError);
        });
    });
    describe('update', () => {
        it('should update title', () => {
            const cell = cell_1.Cell.create({ title: 'Original' });
        ...
        it('should update description', () => {
            const cell = cell_1.Cell.create({ title: 'Test' });
    ...
    describe('status transitions', () => {
        it('should transition from open to in_progress', () => {
            const cell = cell_1.Cell.create({ title: 'Test', status: 'open' });
        ...
        it('should throw on invalid transition', () => {
            const cell = cell_1.Cell.create({ title: 'Test', status: 'open' });
            expect(() => cell.transitionStatus('completed')).toThrow(cell_1.InvalidStatusTransitionError);
        });
        it('should allow all valid transitions', () => {
            for (const [fromStatus, toStatuses] of Object.entries(types_1.VALID_STATUS_TRANSITIONS)) {
                for (const toStatus of toStatuses) {
                    const cell = cell_1.Cell.create({
                        title: 'Test',
                        status: fromStatus,
                    });
                    expect(() => cell.transitionStatus(toStatus)).not.toThrow();
    ...
    describe('hierarchy', () => {
        it('should add child', () => {
            const parent = cell_1.Cell.create({ title: 'Parent' });
            const child = cell_1.Cell.create({ title: 'Child' });
        ...
        it('should check if descendant', () => {
            const grandparent = cell_1.Cell.create({ title: 'Grandparent' });
            const parent = cell_1.Cell.create({
                title: 'Parent',
                parentId: grandparent.id,
            });
            const child = cell_1.Cell.create({
                title: 'Child',
                parentId: parent.id,
            });
            const getCell = (id) => {
            ...
            expect(child.isDescendantOf(grandparent.id, getCell)).toBe(true);
            expect(child.isDescendantOf(parent.id, getCell)).toBe(true);
            expect(parent.isDescendantOf(grandparent.id, getCell)).toBe(true);
            expect(grandparent.isDescendantOf(child.id, getCell)).toBe(false);
        });
    });
    describe('clone', () => {
        ...
        it('should reset history and children on clone', () => {
            const original = cell_1.Cell.create({ title: 'Original' });
        ...
        it('should apply overrides when cloning', () => {
            const original = cell_1.Cell.create({ title: 'Original', priority: 5 });
    ...
    describe('serialization', () => {
        it('should serialize to JSON', () => {
            const cell = cell_1.Cell.create({ title: 'Test', priority: 3 });
...
describe('generateCellId', () => {
    it('should generate unique IDs', () => {
        const ids = new Set();
        for (let i = 0; i < 100; i++) {
            ids.add((0, cell_1.generateCellId)());
        }
        expect(ids.size).toBe(100);
    });
    it('should include type prefix', () => {
        expect((0, cell_1.generateCellId)('epic').startsWith('epic-')).toBe(true);
        expect((0, cell_1.generateCellId)('bug').startsWith('bug-')).toBe(true);
        expect((0, cell_1.generateCellId)('task').startsWith('task-')).toBe(true);
    });
});

...
// Test context helper
const createGuardrailContext = (): GuardrailContext => ({
...
describe('BaseGuardrail', () => {
  ...
  class TestGuardrail extends BaseGuardrail<string, string> {
    readonly name = 'TestGuardrail';
    readonly priority = 5;
    readonly description = 'Test guardrail';
    shouldThrow = false;
    shouldBlock = false;
  ...
  describe('preprocess hook', () => {
    class PreprocessGuardrail extends BaseGuardrail<string, string> {
      readonly name = 'PreprocessGuardrail';
      readonly priority = 5;
      readonly description = 'Test with preprocess';

      protected preprocess(input: string): string {
...
describe('ContentSafetyGuardrail', () => {
  let guardrail: ContentSafetyGuardrail;
  let context: GuardrailContext;
  ...
  describe('configuration', () => {
    it('should respect custom blocked categories', async () => {
      const customGuardrail = new ContentSafetyGuardrail(['harassment']);
...
describe('CompositeGuardrail', () => {
  ...
  describe('priority ordering', () => {
    it('should run guards in priority order', async () => {
      ...
      class OrderTrackingGuardrail extends BaseGuardrail<string, string> {
        constructor(
        ...
        readonly description = 'Order tracking';
      ...
      const composite = new CompositeGuardrail([guard1, guard2, guard3]);
  ...
  describe('blocking behavior (mode=all)', () => {
    it('should block when any guard blocks', async () => {
      const promptGuard = new PromptInjectionGuardrail();
      const contentGuard = new ContentSafetyGuardrail();

      const composite = new CompositeGuardrail([promptGuard, contentGuard]);
  ...
  describe('metadata aggregation', () => {
    ...
    it('should include guard names in results', async () => {
      ...
      const results = result.metadata?.guardrailResults as Array<{ name: string }>;
  ...
  describe('severity calculation', () => {
    it('should return highest severity from results', async () => {
      class HighSeverityGuardrail extends BaseGuardrail<string, string> {
        readonly name = 'HighSeverity';
        readonly priority = 5;
        readonly description = 'Returns high severity';
      ...
      class CriticalSeverityGuardrail extends BaseGuardrail<string, string> {
        readonly name = 'CriticalSeverity';
        readonly priority = 1;
        readonly description = 'Returns critical severity';
      ...
      const composite = new CompositeGuardrail([
  ...
  describe('introspection', () => {
    ...
    it('should return execution mode', () => {
      const composite = new CompositeGuardrail([], 'any');
...
describe('GuardrailBlockedError', () => {
  ...
  it('should default to high severity', () => {
    const error = new GuardrailBlockedError('Test', 'Reason');
...
describe('GuardrailResult severity levels', () => {
  it('should support all severity levels', async () => {
    class SeverityTestGuardrail extends BaseGuardrail<string, string> {
      severity: 'low' | 'medium' | 'high' | 'critical' = 'low';
      readonly name = 'SeverityTest';
      readonly priority = 5;
      readonly description = 'Test severities';
